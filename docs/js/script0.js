...
if (typeof marked === 'undefined') {
  console.error("❌ ERREUR: marked n'est pas chargé !");
  alert("ERREUR: La librairie 'marked' n'est pas disponible. Le script ne peut pas fonctionner.");
} else {
  console.log("✅ marked est disponible !");
  console.log("Version de marked:", marked.version || "inconnue");
}
...
if (!textarea) console.error("❌ textarea non trouvé !");
if (!preview) console.error("❌ preview non trouvé !");
if (!template) console.error("❌ template non trouvé !");
if (!theme) console.error("❌ theme non trouvé !");
...
function processAlerts(md) {
  ...
  return md.replace(regex, (match, header, body) => {
    console.log("🚨 Traitement d'une alerte");
    ...
    console.log("✅ Alerte convertie:", title);
...
function processCallouts(md) {
  ...
  return md.replace(regex, (match, header, body) => {
    console.log("📢 Traitement d'un callout");
    const parts = header.split('\n');
    const title = parts[0].trim();
    const optionsText = parts.slice(1).join('\n');
    const options = parseOptions(optionsText);
    ...
    // CORRECTION: Parser le contenu en Markdown
    const htmlContent = marked.parse(content);
    console.log("✅ Callout converti:", title);
...
function processAccordions(md) {
  ...
  return md.replace(regex, (match, header, body) => {
    accordionId++;
    console.log("📋 Traitement d'un accordéon #" + accordionId);
    const parts = header.split('\n');
    const title = parts[0].trim();
    const optionsText = parts.slice(1).join('\n');
    const options = parseOptions(optionsText);
    ...
    const htmlContent = marked.parse(content);
    console.log("✅ Accordéon converti:", title, "| Contenu HTML:", htmlContent.substring(0, 50));
...
function processBadges(md) {
  ...
  return md.replace(regex, (match, body) => {
    console.log("🏷️ Traitement d'un badge");
...
function processCards(md) {
  ...
  return md.replace(regex, (match, header, body) => {
    console.log("🎴 Traitement d'une carte");
    const parts = header.split('\n');
    const title = parts[0].trim();
    const optionsText = parts.slice(1).join('\n');
    const options = parseOptions(optionsText);
...
function processTiles(md) {
  ...
  return md.replace(regex, (match, header, body) => {
    console.log("🎯 Traitement d'une tuile");
    const parts = header.split('\n');
    const title = parts[0].trim();
    const optionsText = parts.slice(1).join('\n');
    const options = parseOptions(optionsText);
...
function processGrids(md) {
  console.log("📐 Traitement des grilles");
...
function processAllComponents(md) {
  console.log("⚙️ Traitement de tous les composants...");
  ...
  console.log("✅ Tous les composants traités !");
  return result;
}
...
console.log("🔧 Configuration des boutons de la toolbar...");
...
const dataInsertButtons = document.querySelectorAll('[data-insert]');
console.log(`✅ ${dataInsertButtons.length} boutons data-insert trouvés`);

dataInsertButtons.forEach(button => {
  button.addEventListener('click', (e) => {
    e.stopPropagation();
    const textToInsert = button.getAttribute('data-insert');
    
    console.log("🖱️ Clic sur bouton, insertion de:", textToInsert.substring(0, 50) + "...");
...
console.log("✅ Script entièrement chargé et configuré !");
